...
Return the result table in any order. (The product that was only sold in first quarter and not before or after in given timeframe ever)
...
where s.product_id IN (select s.product_id 
						from #Sales s 
						where s.sale_date >= '2019-01-01' and sale_date <= '2019-03-31')
and s.product_id NOT IN (select s.product_id 
							from #Sales s 
							where s.sale_date < '2019-01-01' OR sale_date > '2019-03-31')
/*************************************************************************/
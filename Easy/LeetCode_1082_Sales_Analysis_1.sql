...
from (select s.seller_id,
		...
		from #Sales s 
		group by s.seller_id) sl
where rk = 1
/*************************************************************************/

/******************************Solution # 2*******************************/
select seller_id
from (select s.seller_id, SUM(s.price) totalPrice
		from #Sales s 
		group by s.seller_id) a
where a.totalPrice = (select MAX(b.[sum]) from (select s1.seller_id, SUM(s1.price) [sum] 
						from #Sales s1
					  group by s1.seller_id)b)			
/*************************************************************************/
